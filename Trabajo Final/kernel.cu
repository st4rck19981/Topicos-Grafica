...
__global__ void CUDA(unsigned char* In, char* Mask, unsigned char* Out, int Mask_Width, int Rowimg, int Colimg, bool media) {
	...
	suma = Pvalue;
	if (media == true)	suma = suma / 9;
	Out[row * Rowimg + col] = clamp(suma);
}

void callKernel2D(unsigned char* img, unsigned char* imgOut,char* h_Mask,int Mask_Width, int Row, int Col, bool media) {
//problemas con mask_size?
	...
	CUDA<< <dimGrid, dimBlock >> > (d_In, d_Mask, d_Out, Mask_Width, Row, Col, media);
